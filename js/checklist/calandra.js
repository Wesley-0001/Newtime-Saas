...
const TOTAL = 6;
const COLLECTION = 'checklist_calandra';
...
function init() {
  ...
  const form = document.getElementById('form-calandra');
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      console.warn('[Calandra checklist] persistência indisponível — sucesso visual (demo):', err && (err.message || err));
    }

    const simulated = !persisted;
    appendCheckinToMatrizDemoRows(matrizRowFromCalandraOrImpressao('Calandra', base, recordedAt, simulated));

    form.style.display = 'none';

    mountCheckinSuccessUI({
      successPanel,
      sectorLabel: 'Calandra',
      sectorKey: 'calandra',
      equipeLabel,
      accentClass: 'nt-checkin-accent--calandra',
  ...
  if (darkToggle) {
    darkToggle.addEventListener('change', () => {
      ...
      try {
        localStorage.setItem('calandra-checklist-dark', darkToggle.checked ? '1' : '0');
      } catch {
        /* ignore */
      }
    });
    try {
      if (localStorage.getItem('calandra-checklist-dark') === '1') {
